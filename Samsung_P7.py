...
def clean_data(df):
    # 1. Fill missing CurrentStock with median
    df['CurrentStock'] = df['CurrentStock'].fillna(df['CurrentStock'].median())

    # 2. Standardize Supplier Name (uppercase and strip spaces) to prevent duplicates
    df['SupplierName'] = df['SupplierName'].str.strip().str.upper()

    # 3. Ensure ForecastQuantity is not negative
    df['ForecastQuantity'] = df['ForecastQuantity'].apply(lambda x: x if x >= 0 else 0)

    # 4. Remove duplicate rows based on ProductID
...
def analyze_supply_risk(df):
    ...
    # Calculate Revenue Risk: If Gap < 0 (shortage), multiply missing quantity by unit price
    df['Revenue_Risk'] = np.where(df['Supply_Gap'] < 0, abs(df['Supply_Gap']) * df['UnitPrice'], 0)

    # Filter products with shortages and sort by revenue risk in descending order
...
# Print results to console (Terminal)
